...
export class MatchingService {
     ...
     */
    async findWorkersForCriteria(criteria: MatchCriteria): Promise<import('../types').PaginatedResult<WorkerMatch>> {
        try {
            logger.info('Finding workers for criteria:', criteria);

            if (!criteria.location) {
                logger.warn('No location provided in criteria');
                return { matches: [], totalCount: 0, page: 1, limit: 20, totalPages: 1 };
            }
            const criteriaLocation = await geocodeLocation(criteria.location);
            if (!criteriaLocation) {
                logger.warn('Could not geocode criteria location');
                return { matches: [], totalCount: 0, page: 1, limit: 20, totalPages: 1 };
            }
            ...
            const result = await pool.query(query, [criteria.skillType]);
            const workers = result.rows || [];
            if (workers.length === 0) {
                logger.info('No workers found for criteria');
                return { matches: [], totalCount: 0, page: 1, limit: 20, totalPages: 1 };
            }
            ...
            for (const worker of workers) {
                ...
                const workerLocation = await geocodeLocation(worker.location);
                if (!workerLocation) continue;

                const distance = calculateDistance(criteriaLocation, workerLocation);
                ...
                if (criteria.experienceLevel) {
                    ...
                    if (
                        (criteria.experienceLevel === 'beginner' && exp > 2) ||
                        (criteria.experienceLevel === 'intermediate' && (exp < 3 || exp > 7)) ||
                        (criteria.experienceLevel === 'expert' && exp < 8)
                    ) continue;
                }
                if (criteria.rating && worker.rating < criteria.rating) continue;
                if (criteria.availability && worker.availability_status !== criteria.availability) continue;

                // Apply budget filter if specified
                if (criteria.budgetRange && worker.hourly_rate > criteria.budgetRange.max) continue;

                const matchScore = calculateWorkerMatchScore(worker, workerLocation, criteria, criteriaLocation, criteria.weights);
            ...
            const filteredMatches = filterByMinimumScore(sortedMatches, 30);
            ...
            logger.error('Error finding workers for criteria:', error);
            return { matches: [], totalCount: 0, page: 1, limit: 20, totalPages: 1 };
        }
    }

    // Find contractors matching criteria (for workers looking for jobs)
    async findContractorsForCriteria(criteria: MatchCriteria): Promise<import('../types').PaginatedResult<ContractorMatch>> {
        try {
            logger.info('Finding contractors for criteria:', criteria);
            ...
            for (const contractor of contractors) {
                const contractorLocation = await geocodeLocation(contractor.location);
                if (!contractorLocation) continue;

                const distance = calculateDistance(criteriaLocation, contractorLocation);

                if (distance <= criteria.maxDistance) {
                    const matchScore = calculateContractorMatchScore(contractor, contractorLocation, criteria, criteriaLocation, criteria.weights);
            ...
            logger.error('Error finding contractors for criteria:', error);
            throw error;
        }
    }

    // Get user's match preferences
    async getUserMatchPreferences(userId: string): Promise<MatchPreferences | null> {
        try {
    ...
    async upsertMatchPreferences(userId: string, preferences: UpdateMatchPreferencesRequest): Promise<MatchPreferences> {
        try {
            ...
            logger.error('Error upserting match preferences:', error);
            throw error;
        }
    }

    // Save a match for later reference
    async saveMatch(userId: string, matchData: SaveMatchRequest): Promise<SavedMatch> {
        try {
            const query = `
    ...
    async getSavedMatches(userId: string): Promise<SavedMatch[]> {
        try {
            const result = await pool.query(
                `SELECT sm.*, u.name as matched_user_name, u.role as matched_user_role,
                        wp.is_available
                     ...
                     WHERE sm.user_id = $1 
                     ORDER BY sm.is_favorite DESC, sm.created_at DESC`,
                [userId]
            );
            return result.rows;
        } catch (error) {
    ...
    async deleteSavedMatch(userId: string, matchId: string): Promise<void> {
        try {
            const result = await pool.query(
                'DELETE FROM saved_matches WHERE id = $1 AND user_id = $2',
                [matchId, userId]
            );
